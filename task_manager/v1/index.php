...
$app->post('/register', function() use ($app) {
            ...
            verifyRequiredParams(array('trainid', 'name', 'password', 'email', 'zone'));
            ...
            $db = new DbHandler();
            $res = $db->createUser($trainid, $tcid, $name, $password, $email, $zone);
...
//             // check for required params
//             verifyRequiredParams(array('trainid', 'last_date', 'last_time', 'no_of_penalty'));

//             $response = array();
//             $trainid = $app->request->post('trainid');
//             $last_date = $app->request->post('last_date');
//             $last_time = $app->request->post('last_time');
//             $no_of_penalty = $app->request->post('no_of_penalty');
...
//             // adding new train
//             $train_id = $db->createTrain($train, $last_date, $last_time, $no_of_penalty);
...
$app->put('/trains/:id', function($trainid) use($app) {
            // check for required params
            verifyRequiredParams(array('last_date', 'last_time', 'no_of_penalty'));
            ...
            // updating train
            $result = $db->updateTrain($trainid, $last_date, $last_time, $no_of_penalty);
            if ($result) {
 ...
 * url /trains         
 */
 $app->get('/coaches/:id', function($trainid) {
    $response = array();
    $db = new DbHandler();
 ...
 * url - /coaches/:id
 */
 $app->put('/coaches/:id', function($coachid) use($app) {
    // check for required params
    verifyRequiredParams(array('no_of_penalty'));
    ...
    // updating coach
    $result = $db->updateCoach($no_of_penalty);
    if ($result) {
        ...
        //  failed to update
        $response["error"] = true;
        $response["message"] = "Coach failed to update. Please try again!";
    }
    echoRespnse(200, $response);
});

/*
*Listing all passengers of a coach
*/
$app->get('/passengers/:id', function($coachid) {
    $response = array();
    $db = new DbHandler();

    // fetching all user coaches
    $result = $db->getAllUserCoaches($coachid);
    ...
    while ($coach = $result->fetch_assoc()) {
        ...
        $tmp["name"] = $coach["name"];
        $tmp["coachid"] = $coach["coachid"];
        $tmp["penalty"] = $coach["penalty"];
        $tmp["start"] = $coach["start"];
        $tmp["end"] = $coach["end"];
        $tmp["pid"] = $coach["pid"];
