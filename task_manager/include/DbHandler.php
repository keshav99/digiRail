...
class DbHandler {
    ...
    public function createUser($trainid, $tcid, $name, $email, $zone) {
        ...
        if (!$this->isUserExists($email)) {
            ...
            // insert query
            $stmt = $this->conn->prepare("INSERT INTO tc(trainid, tcid, name, password, email, zone, api_key) values(?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssssss", $trainid, $tcid, $name, $password, $email, $zone, $api_key);
    ...
    public function checkLogin($email, $password) {
        ...
        $stmt = $this->conn->prepare("SELECT password FROM tc WHERE email = ?");
    ...
    /**
     * Fetching all users in a coach
     * @param String $user_id id of the user
     */
     public function getAllUserCoaches($coachid) {
        $stmt = $this->conn->prepare("SELECT * FROM passengers where coachid = ?");
     ...
     */
    public function updateTrain($last_date, $last_time, $no_of_penalty) {
        $stmt = $this->conn->prepare("UPDATE trains t set t.last_date = ?, t.last_time = ?, t.no_of_penalty = ? WHERE t.trainid = ?");
        $stmt->bind_param("ssis", $last_date, $last_time, $no_of_penalty, $trainid);
        $stmt->execute();
     ...
     * @param String $status train status
     */
     public function updateCoach($no_of_penalty) {
        $stmt = $this->conn->prepare("UPDATE coaches set no_of_penalty = ? WHERE coachid = ?");
        $stmt->bind_param("is", $no_of_penalty, $trainid);
        $stmt->execute();
        $num_affected_rows = $stmt->affected_rows;
        $stmt->close();
        return $num_affected_rows > 0;
